package com.brec_project.components.graphs;
...
import com.brec_project.MainApplication;
import com.brec_project.components.csv.EEGFileReader;
import com.brec_project.components.csv.EEGFileWriter;
import com.brec_project.components.signal.CircularBuffer;
import com.brec_project.components.signal.Filter;
...
public class FilterGraph extends FrameLayout {
    ...
    public static final int BACKGROUND_COLOUR = Color.rgb(0, 150, 136);
    ...
    public void startRecording(String title, String header) {
        fileWriter.initFile(title, header); //Add header
        isRecording = true;
    }

    public void stopRecording(String title) {
        isRecording = false;
        // if writer = writing, close and save file
        if (fileWriter.isRecording()) {
            fileWriter.writeFile(title);
        }
    }
    ...
    public void initView(Context context) {
        ...
        filterPlot.getGraph().getBackgroundPaint().setColor(BACKGROUND_COLOUR);
