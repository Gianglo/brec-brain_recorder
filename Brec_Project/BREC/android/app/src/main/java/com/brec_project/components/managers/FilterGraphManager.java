package com.brec_project.components.managers;

import com.brec_project.components.database.DatabaseModule;
import com.brec_project.components.graphs.FilterGraph;
...
public class FilterGraphManager extends SimpleViewManager<FilterGraph> {
    ...
    public void setIsRecording(FilterGraph graph, @Nullable boolean isRecording) {
        if(isRecording) {
            graph.startRecording(DatabaseModule.getTitle(), DatabaseModule.getHeader());
        } else {
            graph.stopRecording(DatabaseModule.getTitle());
        }
    }
